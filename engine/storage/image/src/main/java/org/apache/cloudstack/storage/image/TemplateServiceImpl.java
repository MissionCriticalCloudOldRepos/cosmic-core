...
public class TemplateServiceImpl implements TemplateService {
  ...
  public void createTemplateAsync(TemplateInfo template, DataStore store, AsyncCompletionCallback<TemplateApiResult> callback) {
    ...
    try {
      final TemplateOpContext<TemplateApiResult> context = new TemplateOpContext<TemplateApiResult>(callback, templateOnStore, null);
  ...
  public void handleSysTemplateDownload(HypervisorType hostHyper, Long dcId) {
    final Set<VMTemplateVO> toBeDownloaded = new HashSet<VMTemplateVO>();
        ...
        try {
          ...
          if (toBeDownloaded.size() > 0) {
            ...
            for (final VMTemplateVO tmplt : toBeDownloaded) {
              ...
              // if this is a region store, and there is already an DOWNLOADED entry there without install_path
              // information, which
          ...
          for (final String uniqueName : templateInfos.keySet()) {
            ...
            if (_tmpltMgr.templateIsDeleteable(tInfo.getId())) {
              // we cannot directly call deleteTemplateSync here to reuse delete logic since in this case db does not
              // have this template at all.
  ...
  public void associateTemplateToZone(long templateId, Long zoneId) {
    final List<Long> dcs = new ArrayList<Long>();
    if (zoneId != null) {
      dcs.add(zoneId);
    } else {
  ...
  public AsyncCallFuture<TemplateApiResult> deleteTemplateAsync(TemplateInfo template) {
    ...
    final AsyncCallFuture<TemplateApiResult> future = new AsyncCallFuture<TemplateApiResult>();

    final TemplateOpContext<TemplateApiResult> context = new TemplateOpContext<TemplateApiResult>(null, to, future);
  ...
  private AsyncCallFuture<TemplateApiResult> copyAsync(DataObject source, TemplateInfo template, DataStore store) {
    ...
    final TemplateOpContext<TemplateApiResult> context = new TemplateOpContext<TemplateApiResult>(null, (TemplateObject) templateOnStore, future);
  ...
  private AsyncCallFuture<TemplateApiResult> syncToRegionStoreAsync(TemplateInfo template, DataStore store) {
    ...
    final String installPath = TemplateConstants.DEFAULT_TMPLT_ROOT_DIR + "/" + TemplateConstants.DEFAULT_TMPLT_FIRST_LEVEL_DIR + template.getAccountId() + "/" + template.getId() + "/"
        +
  ...
  protected Void syncTemplateCallBack(AsyncCallbackDispatcher<TemplateServiceImpl, CopyCommandResult> callback, TemplateOpContext<TemplateApiResult> context) {
    ...
    try {
      if (result.isFailed()) {
        ...
        // no change to existing template_store_ref, will try to re-sync later if other call triggers this sync
        // operation, like copy template
      } else {
  ...
  public void syncTemplateToRegionStore(long templateId, DataStore store) {
    if (_storeMgr.isRegionStore(store)) {
      ...
      // if template is on region wide object store, check if it is really downloaded there (by checking install_path).
      // Sync template to region
      ...
      if (tmplOnStore.getInstallPath() == null || tmplOnStore.getInstallPath().length() == 0) {
        ...
        try {
          ...
          _cacheMgr.releaseCacheObject(srcTemplate); // reduce reference count for template on cache, so it can recycled
          // by schedule
