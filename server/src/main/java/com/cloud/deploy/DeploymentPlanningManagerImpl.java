...
StateListener<State, VirtualMachine.Event, VirtualMachine> {
    ...
    if (plan.getHostId() != null && haVmTag == null) {
      ...
      if (host == null) {
        ...
        if (!suitableVolumeStoragePools.isEmpty()) {
          final List<Host> suitableHosts = new ArrayList<Host>();
          ...
          if (potentialResources != null) {
            ...
            final DeployDestination dest = new DeployDestination(dc, pod, cluster, host, storageVolMap);
  ...
  private void checkForNonDedicatedResources(VirtualMachineProfile vmProfile, DataCenter dc, ExcludeList avoids) {
    ...
    final DedicatedResourceVO dedicatedZone = _dedicatedDao.findByZoneId(dc.getId());
    ...
    if (vm.getType() == VirtualMachine.Type.DomainRouter) {
      ...
      final List<Long> allPodsFromDedicatedID = new ArrayList<Long>();
      final List<Long> allClustersFromDedicatedID = new ArrayList<Long>();
      final List<Long> allHostsFromDedicatedID = new ArrayList<Long>();
      ...
      if (suitableHosts != null && !suitableHosts.isEmpty()) {
        ...
        if (!suitableVolumeStoragePools.isEmpty()) {
          ...
          if (potentialResources != null) {
            ...
            final DeployDestination dest = new DeployDestination(dc, pod, clusterVO, host, storageVolMap);
            s_logger.debug("Returning Deployment Destination: " + dest);
            return dest;
          }
        } else {
  ...
  protected Pair<Host, Map<Volume, StoragePool>> findPotentialDeploymentResources(List<Host> suitableHosts, Map<Volume, List<StoragePool>> suitableVolumeStoragePools,
    ...
    final Map<Volume, StoragePool> storage = new HashMap<Volume, StoragePool>();
    final TreeSet<Volume> volumesOrderBySizeDesc = new TreeSet<Volume>(new Comparator<Volume>() {
      @Override
    ...
    for (final Host potentialHost : suitableHosts) {
      final Map<StoragePool, List<Volume>> volumeAllocationMap = new HashMap<StoragePool, List<Volume>>();
  ...
  protected Pair<Map<Volume, List<StoragePool>>, List<Volume>> findSuitablePoolsForVolumes(VirtualMachineProfile vmProfile, DeploymentPlan plan, ExcludeList avoid,
      int returnUpTo) {
    final List<VolumeVO> volumesTobeCreated = _volsDao.findUsableVolumesForInstance(vmProfile.getId());
    final Map<Volume, List<StoragePool>> suitableVolumeStoragePools = new HashMap<Volume, List<StoragePool>>();
    final List<Volume> readyAndReusedVolumes = new ArrayList<Volume>();
    ...
    final Set<Long> poolsToAvoidOutput = new HashSet<Long>(originalAvoidPoolSet);
